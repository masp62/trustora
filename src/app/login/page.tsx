import Link from "next/link";

import { googleAuthConfigured } from "@/auth";
import { signInWithGoogle } from "@/lib/auth-actions";

import { LoginForm } from "./login-form";

export default function LoginPage() {
  return (
    <main className="flex flex-1 items-center px-4 py-12 sm:px-6 lg:px-8 xl:px-10 2xl:px-12">
      <section className="mx-auto grid w-full max-w-[1760px] gap-8 rounded-[2rem] border border-stone-200/80 bg-white/90 p-8 shadow-xl shadow-amber-950/10 backdrop-blur sm:grid-cols-2 sm:p-12">
        <div>
          <p className="text-sm font-semibold tracking-[0.15em] text-amber-700 uppercase">Welcome back</p>
          <h1 className="mt-4 font-heading text-4xl leading-tight text-stone-900 sm:text-5xl">
            Sign in to RealBnB
          </h1>
          <p className="mt-4 text-stone-700">
            Continue with Google or use your email and password.
          </p>

          {googleAuthConfigured ? (
            <form action={signInWithGoogle} className="mt-8">
              <button
                type="submit"
                className="inline-flex w-full items-center justify-center rounded-full border border-amber-300 bg-amber-50 px-6 py-3 font-semibold text-amber-900 transition hover:bg-amber-100"
              >
                Continue with Google
              </button>
            </form>
          ) : (
            <p className="mt-8 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
              Google sign-in is not configured for this environment.
            </p>
          )}
        </div>

        <div className="rounded-2xl border border-amber-100 bg-white p-6">
          <LoginForm />
          <p className="mt-5 text-sm text-stone-600">
            No account yet?{" "}
            <Link href="/signup" className="font-semibold text-amber-700 hover:text-amber-600">
              Create one
            </Link>
          </p>
        </div>
      </section>
    </main>
  );
}
